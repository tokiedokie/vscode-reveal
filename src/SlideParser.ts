...
export const parseSlides = (
  slideContent: string,
  slidifyOptions: IDocumentOptions
): ISlide[] => {
...
const lineInSeparator = (separator: string) =>
  (separator.match(/\\n/gm) || []).length + 1

export const countLinesToSlide = (
  slides: ISlide[],
  horizontalIndex: number,
  verticalIndex: number,
  slidifyOptions: IDocumentOptions
) => {
  const stopSlideIndex =
    verticalIndex > 0 ? horizontalIndex + 1 : horizontalIndex
  const separatorHeight = lineInSeparator(slidifyOptions.separator)
  const verticalSeparatorHeight = lineInSeparator(
    slidifyOptions.verticalSeparator
  )
...
const addChildrenSlideLines = (
  slide,
  horizontalIndex,
  verticalIndex,
  verticalSeparatorHeight
) => {
  const stopVerticalAt =
    slide.index === horizontalIndex
      ? verticalIndex - 1
      : slide.verticalChildren.length

  return slide.verticalChildren
    ? slide.verticalChildren
        .slice(0, stopVerticalAt)
        .reduce((innerLines, innerSlide) => {
          return (
            innerLines + countLines(innerSlide.text) + verticalSeparatorHeight
          )
        }, 0)
    : 0
}
...
const parseSlide = (
  slideContent: string,
  index: number,
  documentOption: IDocumentOptions
): ISlide => {
