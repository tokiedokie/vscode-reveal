...
export function activate(context: ExtensionContext): void {
  ...
  commands.executeCommand(
    'setContext',
    'slideExplorerEnabled',
    container.configuration.slideExplorerEnabled
  )

  registerCommand(
    SHOW_REVEALJS,
    showRevealJS((view) => container.refreshWebView(view))
  )
  registerCommand(
    SHOW_REVEALJS_IN_BROWSER,
    showRevealJSInBrowser(() => container.getUri(), getBrowser)
  )
  registerCommand(STOP_REVEALJS_SERVER, () => container.stopServer())

  registerCommand(GO_TO_SLIDE, (arg) =>
    container.goToSlide(arg.horizontal, arg.vertical)
  )
  registerCommand(
    EXPORT_PDF,
    exportPDF(() => container.getUri(false), getBrowser)
  )
  registerCommand(
    EXPORT_HTML,
    exportHTML(
      logger,
      container.export,
      () => container.configuration.openFilemanagerAfterHTMLExport
    )
  )

  window.onDidChangeTextEditorSelection((e) =>
    container.onDidChangeTextEditorSelection(e)
  )
  window.onDidChangeActiveTextEditor((e) =>
    container.onDidChangeActiveTextEditor(e)
  )
  ...
  workspace.onDidChangeConfiguration((e) =>
    container.onDidChangeConfiguration(e)
  )
}
